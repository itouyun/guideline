...
\
 ...
 .. note:: **@Transactionalアノテーションのtimeout属性について**
    ...
    トランザクション開始時点から\ ``@Transactional``\アノテーションのtimeout属性に指定した時間が経過するとタイムアウトであり、クエリを発行する時にタイムアウトしていないかチェックを行う。
    タイムアウトチェック時に既にタイムアウトしていないかを確認するため、timeout属性に指定した時間が経過したタイミングで例外が発生するわけではない。
    また、タイムアウトチェック後に、関係ない業務処理にいくら時間がかかってもタイムアウトにはならない。クエリを発行した後のタイムアウトの挙動はJDBCドライバの実装に依存する。

    なお、使用するTransaction Managerによっては、クエリを発行する時とは別のタイムアウトチェックのタイミングが存在する。
